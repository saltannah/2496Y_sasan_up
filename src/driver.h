...
int flywheel_control(int time)
{
    ...
    int flat_speeds[] = {400, 375}; //400, 370
...
void intake_control(int speed_index)
{
    ...
    {
        shoot_speed = speed_index == 0 ? 127 : 100;
    }
    else
    {
        shoot_speed = speed_index == 0 ? 127 : 100;
...
void print_info(int time, bool chassis_on)
{
    ...
    {
        if(chassis_on) con.print(0, 0, "%.1lf:%.1lf:%.1lf         ", chas.temp(), (glb::flywheel.get_temperature()), (glb::intake.get_temperature()));
